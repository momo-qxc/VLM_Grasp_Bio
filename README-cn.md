...
基于多模态视觉语言模型（VLM）的机械臂抓取与放置演示工程。项目在 MuJoCo 中模拟 UR5e 机械臂，通过 **全自然语言指令 + 视觉分割 + 点云抓取推理** 完成目标物体的抓取与放置；支持 **单相机**、**双相机点云融合** 以及 **智能放置** 三种模式，实现端到端的自然语言控制。

## 功能概览
- **全自然语言控制**：支持完整的自然语言指令，如"把培养皿放置到显微镜的右边"，实现端到端的抓取与放置。
- **VLM 目标定位**：输入自然语言指令，VLM 输出目标物体的 2D 边界框。
- **智能放置检测**：基于空间描述，VLM 从多相机视角识别放置位置。
...
- `3`：双相机点云融合（推荐用于遮挡场景）
- `4`：**智能放置模式** 🔥 - 全自然语言控制抓取与放置

### 模式 4：智能放置
在此模式下，可以输入完整的自然语言指令，例如：
- "把培养皿放置到显微镜的右边"
- "把试管移到桌子左上角"
- "抓取烧杯放到红色区域"

系统将：
1. 解析指令，提取抓取目标和放置位置描述
2. 使用 VLM 从桌面相机分割目标物体
3. 通过 GraspNet 推理抓取姿态
4. 使用多相机视角（全局相机）识别放置位置
5. 执行完整的抓取与放置任务

融合模式（模式 3）下会提示输入目标物体的 **自然语言指令**，随后进行 VLM 分割与抓取推理。
...
![交互界面2](Visual%20results/交互界面2.png)
